...
public interface MentorMapper {
	...
	@Select("SELECT a.id,a.name,a.mentorName,a.skill,a.startDate,a.endDate,DATEDIFF(a.endDate, a.startDate) as duration,a.fee, a.description, a.userName FROM sba_course.course a where  a.status='available' and a.userName is null and ( (disabled = 1 and (disable_startDate > DATE_FORMAT(sysdate(),'%y-%c-%d') or disable_endDate < DATE_FORMAT(sysdate(),'%y-%c-%d')) ) or disabled = 0 )")
